...
function App() {
  ...
  // 이 함수를 이용해서 limitNum만큼 displayedPokemons state에 포켓몬 추가
  ...
  const handleSearchInput = async(searchTerm) => { //타이핑을 할때마다 작동하는 비동기 이벤트 
          ...
          {displayedPokemons.length > 0 ? 
            ...
            <div className='text-center'>
              {(allPokemons.length > displayedPokemons.length) && (displayedPokemons.length !== 1) && 
                (
                  /* 더 보기 버튼을 보여주려면 */
                  /* 모든 포켓몬 수가 보여주고 있는 포켓몬 수보다 많고, 보여주는 게 하나일 때가 아이여야 함.(검색 결과를 볼 때) */
                  <button 
